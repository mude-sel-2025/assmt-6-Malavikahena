...
dists = {
    ...
    "Lognormal":lognorm(s=0.5, scale=1),                                 # median = 1 # type: ignore
    "Gumbel-1": gumbel_r(loc=0, scale=1)                                   # Gumbel distribution # type: ignore
...
def task3_averaging(N=1000, m_list=[1, 2, 10, 100]):
    for name, dist in dists.items():
        ...
        for ax, m in zip(axes.flat, m_list):
            ...
            ax.set_title(f"For m={m}, theoretical mean (CLT) ~ N({theo_mu:.3f}, {theo_var * m:.3f}/{m} = {theo_var:.3f})\n"
...
if __name__ == "__main__":
    ...
    #task4_variance_scaling() # type: ignore
